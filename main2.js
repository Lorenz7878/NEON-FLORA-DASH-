...
const SHOP_ITEMS=[
  ...
  {id:'trail_gold',type:'trail',key:'gold',cost:60,icon:'✨',name:'Gold',desc:'engine glow'},
  {id:'trail_plasma',type:'trail',key:'plasma',cost:90,icon:'💜',name:'Plasma',desc:'plasma wake'},
  {id:'trail_inferno',type:'trail',key:'inferno',cost:120,icon:'🔥',name:'Inferno',desc:'Blazing exhaust'},
];
...
function drawParallaxGrid(pal,speed,dt){
  ...
  const speedFrac=Math.min(1,(speed-CFG.baseSpeed)/400);
  const baseAlpha=0.045+speedFrac*0.04;
  ctx.strokeStyle=pal.accent;ctx.globalAlpha=baseAlpha;ctx.lineWidth=1;
  for(let x=-gridOff;x<canvas.width+60;x+=60){
    ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x-gridOff*0.35,canvas.height);ctx.stroke();
  }
  for(let y=gridOff%40;y<canvas.height;y+=40){
    ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(canvas.width,y);ctx.stroke();
  }
  // Linee di velocità in deep void
  if(Zones.idx>=3){
    ctx.globalAlpha=speedFrac*0.08;ctx.lineWidth=1.5;
    for(let i=0;i<6;i++){
      const fy=canvas.height*0.1+i*(canvas.height*0.14);
      const len=40+speedFrac*80;
      ctx.strokeStyle=pal.edge;
      ctx.beginPath();ctx.moveTo(canvas.width*0.92,fy);ctx.lineTo(canvas.width*0.92-len,fy+2);ctx.stroke();
    }
  }
  ctx.globalAlpha=1;
}
function drawMenuBackground(t){
  ...
  if(bg.complete&&bg.naturalWidth>0){
    // Sfondo con leggero parallasse lento nel menu
    const ox=Math.floor(Math.sin(menuStarT*0.1)*20);
    ctx.drawImage(bg,ox,0,canvas.width,canvas.height);
    ctx.fillStyle='rgba(5,6,16,0.55)';ctx.fillRect(0,0,canvas.width,canvas.height);
  }else{
    ctx.fillStyle='#050610';ctx.fillRect(0,0,canvas.width,canvas.height);
  }
  drawNebulae(pal);
  menuStarT+=0.016;
  // stelle del menu con twinkle
  const menuStarArr=starLayers[2];
  menuStarArr.forEach(s=>{
    s.twinkle+=0.022;
    s.x-=s.spd*0.25;if(s.x<0)s.x=1;
    const tw=0.7+0.3*Math.sin(s.twinkle);
    ctx.globalAlpha=s.a*tw;ctx.fillStyle=s.x>0.7?'#00ffcc':'#fff';
    if(s.r>1.3){ctx.shadowColor='#fff';ctx.shadowBlur=5;}
...
function showZoneBanner(name){
  ...
  el.style.color=Zones.palette.edge;
  el.style.textShadow=`0 0 20px ${Zones.palette.accent}, 0 0 40px ${Zones.palette.accent}55`;
  el.classList.add('show');
  clearTimeout(showZoneBanner._t);
  showZoneBanner._t=setTimeout(()=>el.classList.remove('show'),3200);
  // Burst di particelle al cambio zona
  for(let i=0;i<3;i++){
    setTimeout(()=>{
      const y=canvas.height*(0.3+Math.random()*0.4);
      spawnFire(canvas.width*0.5,y,6,[Zones.palette.accent,Zones.palette.edge,'#fff']);
    },i*120);
  }
}
...
//  STELLE & SFONDO
// ═══════════════════════════════════════════════
// ── STELLE PARALLAX A 3 LIVELLI + SHOOTING STARS ──
const starLayers=[
  Array.from({length:60},()=>({x:Math.random(),y:Math.random(),r:Math.random()*0.9+0.2,spd:0.02+Math.random()*0.03,a:Math.random()*0.4+0.15,twinkle:Math.random()*Math.PI*2})),
  Array.from({length:55},()=>({x:Math.random(),y:Math.random(),r:Math.random()*1.3+0.4,spd:0.06+Math.random()*0.06,a:Math.random()*0.55+0.25,twinkle:Math.random()*Math.PI*2})),
  Array.from({length:45},()=>({x:Math.random(),y:Math.random(),r:Math.random()*1.8+0.6,spd:0.12+Math.random()*0.10,a:Math.random()*0.7+0.3,twinkle:Math.random()*Math.PI*2})),
];
const shootingStars=[];let shootTimer=0;
function spawnShootingStar(){
  shootingStars.push({x:1.0+Math.random()*0.1,y:Math.random(),vx:-(0.55+Math.random()*0.4),vy:(Math.random()-0.5)*0.18,len:0.08+Math.random()*0.06,life:1,maxLife:1,a:0.8+Math.random()*0.2});
}
function drawStars(speed,pal,dt){
  const t=performance.now()*0.001;
  const layerSpeeds=[0.08,0.22,0.42];
  layerSpeeds.forEach((ls,li)=>{
    starLayers[li].forEach(s=>{
      s.twinkle+=dt*(0.8+li*0.4);
      s.x-=s.spd*(speed/CFG.baseSpeed)*ls;if(s.x<0)s.x=1;
      const tw=0.75+0.25*Math.sin(s.twinkle);
      ctx.globalAlpha=s.a*tw;
      // stelle più grandi hanno un bagliore
      if(s.r>1.4&&li===2){
        ctx.shadowColor=pal.star;ctx.shadowBlur=4;
      }
  ...
  // Shooting stars — solo in zona 0 e 1
  shootTimer-=dt;
  if(shootTimer<=0&&Zones.idx<=1){shootTimer=3+Math.random()*5;spawnShootingStar();}
  for(let i=shootingStars.length-1;i>=0;i--){
    const s=shootingStars[i];
    s.x+=s.vx*dt;s.y+=s.vy*dt;s.life-=dt*0.7;
    if(s.life<=0||s.x<-0.15){shootingStars.splice(i,1);continue;}
    const alpha=s.life*s.a;
    const x1=s.x*canvas.width,y1=s.y*canvas.height;
    const x2=x1+s.len*canvas.width,y2=y1-s.vy/s.vx*s.len*canvas.width;
    const grad=ctx.createLinearGradient(x1,y1,x2,y2);
    grad.addColorStop(0,'rgba(255,255,255,0)');
    grad.addColorStop(0.6,`rgba(255,255,255,${alpha*0.6})`);
    grad.addColorStop(1,`rgba(255,255,255,${alpha})`);
    ctx.save();ctx.strokeStyle=grad;ctx.lineWidth=1.5;ctx.globalAlpha=1;
...
const nebulaSpots=[
  {x:0.18,y:0.28,rx:0.20,ry:0.24,phase:0},
  {x:0.74,y:0.60,rx:0.24,ry:0.18,phase:1.8},
  {x:0.50,y:0.12,rx:0.16,ry:0.13,phase:3.3},
  {x:0.85,y:0.18,rx:0.12,ry:0.15,phase:0.9},
];
function drawNebulae(pal){
  const t=performance.now()*0.0006;
  nebulaSpots.forEach(n=>{
    const pulse=0.85+0.15*Math.sin(t+n.phase);
    const g=ctx.createRadialGradient(n.x*canvas.width,n.y*canvas.height,0,n.x*canvas.width,n.y*canvas.height,n.rx*canvas.width*pulse);
    g.addColorStop(0,pal.nebula);
    g.addColorStop(0.5,pal.nebula.replace(/[\d.]+\)$/,v=>String(parseFloat(v)*0.4)+')'));
    g.addColorStop(1,'rgba(0,0,0,0)');
    ctx.fillStyle=g;
    ctx.beginPath();ctx.ellipse(n.x*canvas.width,n.y*canvas.height,n.rx*canvas.width*pulse,n.ry*canvas.height*pulse,0,0,Math.PI*2);ctx.fill();
  });
}
function drawBackground(speed,dt){
  ...
  if(bg.complete&&bg.naturalWidth>0){
    // Parallasse: lo sfondo scorre a 0.25x, poi uno strato medio a 0.15x (blur)
    const bx=Math.floor(bgX);
    ctx.drawImage(bg,bx,0,canvas.width,canvas.height);
    ctx.drawImage(bg,bx+canvas.width,0,canvas.width,canvas.height);
    // Overlay colorato della zona corrente — dà identità visiva senza coprire lo sfondo
    const zoneAlpha=0.32+Zones.idx*0.06;
    ctx.fillStyle=pal.bg;ctx.globalAlpha=zoneAlpha;ctx.fillRect(0,0,canvas.width,canvas.height);ctx.globalAlpha=1;
    // Nebulae sopra lo sfondo per profondità
    drawNebulae(pal);
    // Stelle visibili sopra (livello 3 più luminoso)
    drawStars(speed,pal,dt);
  }else{
    const g=ctx.createLinearGradient(0,0,0,canvas.height);
    g.addColorStop(0,pal.bg);g.addColorStop(1,'#020208');
    ctx.fillStyle=g;ctx.fillRect(0,0,canvas.width,canvas.height);
    drawNebulae(pal);drawStars(speed,pal,dt);
  }
  drawParallaxGrid(pal,speed,dt||0.016);
  if(Zones.flash>0){
    ctx.fillStyle=pal.accent;ctx.globalAlpha=Zones.flash*0.28*flashMul;
...
function updateTrail(dt){
  trailTimer+=dt*1000;
  if(trailTimer>=CFG.trailSpacing){
    trailTimer=0;
    trail.push({x:P.hbX,y:P.hbY,age:0,thrusting:P.thrusting});
    if(trail.length>CFG.trailLength)trail.shift();
  }
  trail.forEach(t=>{t.age+=dt;});
}
function drawTrail(){
  ...
  ctx.save();ctx.globalCompositeOperation='lighter';
  // Passata principale — linea continua con spessore variabile
  for(let i=1;i<trail.length;i++){
    const t=trail[i],prev=trail[i-1];
    const prog=i/trail.length;
    const r=prog*P.hbR*(t.thrusting?1.35:0.9);
  ...
  // Passata glow — più morbida, sovrapposta
  ...
  for(let i=1;i<trail.length;i++){
    ...
    ctx.strokeStyle='#fff';
    ctx.lineWidth=prog*P.hbR*0.9;
    ctx.beginPath();ctx.moveTo(prev.x,prev.y);ctx.lineTo(t.x,t.y);ctx.stroke();
  }
  ctx.shadowBlur=0;
  // Skin premium: scintille luminose lungo la scia
  if(Save.trailSkin!=='default'){
    for(let i=2;i<trail.length;i+=3){
      const t=trail[i],prog=i/trail.length;
      if(Math.random()<0.3){
        ctx.globalAlpha=prog*0.7;ctx.fillStyle='#fff';
        ctx.beginPath();ctx.arc(t.x+(Math.random()-0.5)*6,t.y+(Math.random()-0.5)*6,prog*2,0,Math.PI*2);ctx.fill();
      }
    }
  }
  ctx.restore();
...
const Entry={
  ...
  update(dt){
    ...
    this.sparkTimer+=dt;
    const col=CHARACTERS[G.activeChar].trailColor;
    if(this.sparkTimer>0.025){
      this.sparkTimer=0;
      // Scia di lancio
      spawnFire(P.x+P.w*0.1,P.y+P.h*0.55,2,[col,'#fff','#ffeb3b']);
      // Particelle laterali di scia
      parts.spawn(P.x+P.w*0.5,P.y+P.h*0.5,-(120+Math.random()*180),(Math.random()-0.5)*60,col,0.12,1.8);
    }
    if(done){
      // Esplosione di arrivo
      for(let i=0;i<20;i++){
        const a=Math.random()*Math.PI*2,s=40+Math.random()*120;
        parts.spawn(P.hbX,P.hbY,Math.cos(a)*s,Math.sin(a)*s,col,0.25+Math.random()*0.2,2);
      }
      spawnFire(P.hbX,P.hbY,10,[col,'#00ffcc','#ffeb3b','#fff']);
      G.shake=5;
      grantEntryShield();
    }
  }
...
function showScorePop(text,x,y,isCoin,isCombo){
  const el=document.createElement('div');
  // Tier visivo basato sul contenuto del testo
  let tier='';
  if(isCombo){
    if(text.includes('x5')||text.includes('x4'))tier=' mega';
    else tier=' combo';
  }
  el.className='score-pop'+(isCoin?' coin':'')+(isCombo?' combo':'')+tier;
  el.textContent=text;
  el.style.left=(x-20)+'px';el.style.top=(y-10)+'px';
  // Combo alta: più grande e centrata
  if(tier===' mega'){
    el.style.fontSize='1.5rem';el.style.left=(canvas.width/2-40)+'px';
...
class ParticlePool{
  ...
  update(dt){
    for(let p of this.pool){
      ...
      p.vx*=(1-dt*1.2); // drag leggero
      p.vy*=(1-dt*0.8);
      p.x+=p.vx*dt;p.y+=p.vy*dt;
      p.vy+=220*dt; // gravità ridotta per effetto spaziale
      p.life-=dt;
  ...
  draw(){
    ...
    for(let p of this.pool){
      ...
      const r=p.r*(0.3+a*0.7); // shrink con l'età
      ctx.globalAlpha=a*a; // fade quadratico — più morbido
      ctx.shadowColor=p.color;ctx.shadowBlur=r*3;
...
class MeteorPool{
  ...
  draw(){
    ...
    for(let m of this.pool){
      ...
      // Scia del meteorite
      const trailLen=3.5;
      const tx=m.x-m.vx*trailLen/1000,ty=m.y-m.vy*trailLen/1000;
      const trailGrad=ctx.createLinearGradient(tx,ty,m.x,m.y);
      trailGrad.addColorStop(0,'rgba(0,0,0,0)');
      trailGrad.addColorStop(0.6,Zones.palette.accent+'66');
      trailGrad.addColorStop(1,Zones.palette.edge+'cc');
      ctx.strokeStyle=trailGrad;ctx.lineWidth=m.r*0.8;ctx.lineCap='round';
      ...
      g.addColorStop(0.3,Zones.palette.edge);
      g.addColorStop(0.7,Zones.palette.accent);
      g.addColorStop(1,'rgba(0,0,0,0)');
      ctx.fillStyle=g;ctx.shadowColor=Zones.palette.edge;ctx.shadowBlur=12;
...
class ProjectilePool {
  ...
  draw() {
    ...
    for(let p of this.pool) {
      ...
      ctx.fillRect(p.x-4, p.y-p.h*1.5, p.w+8, p.h*4);
      // Corpo principale
      ctx.shadowBlur=10;ctx.fillStyle='#ff1744';
      ctx.fillRect(p.x, p.y, p.w, p.h);
      // Core bianco
      ctx.shadowBlur=0;ctx.fillStyle='#fff';
      ctx.fillRect(p.x+p.w*0.15, p.y+1, p.w*0.7, p.h-2);
      // Punta luminosa
...
//  PARTICELLE
// ═══════════════════════════════════════════════
function spawnFire(x,y,n,colors=['#ff5722','#ffeb3b','#00bcd4']){
  for(let i=0;i<n;i++){
    const angle=(Math.random()-0.5)*Math.PI*1.4-(Math.PI/2);
    const spd=60+Math.random()*220;
    parts.spawn(x,y,Math.cos(angle)*spd,Math.sin(angle)*spd,colors[Math.floor(Math.random()*colors.length)],0.15+Math.random()*0.28,1.5+Math.random()*2);
  }
}
function spawnExplosion(x,y){
  const cols=['#ff5722','#ffeb3b','#ff4444','#fff','#00bcd4','#ff9800'];
  // Onda principale
  for(let i=0;i<CFG.explosionParticles;i++){
    const a=Math.random()*Math.PI*2,s=60+Math.random()*340;
    const sz=2+Math.random()*4;
    parts.spawn(x,y,Math.cos(a)*s,Math.sin(a)*s-40,cols[Math.floor(Math.random()*cols.length)],0.4+Math.random()*0.55,sz);
  }
  // Scintille veloci
  for(let i=0;i<18;i++){
    const a=Math.random()*Math.PI*2,s=280+Math.random()*420;
    parts.spawn(x,y,Math.cos(a)*s,Math.sin(a)*s,'#fff',0.2+Math.random()*0.25,1.2);
  }
  // Onde di detriti lenti
  for(let i=0;i<12;i++){
    const a=Math.random()*Math.PI*2,s=20+Math.random()*80;
    parts.spawn(x,y,Math.cos(a)*s,Math.sin(a)*s,'#ff7043',0.6+Math.random()*0.4,4+Math.random()*3);
  }
}
function spawnCoinPop(x,y){
  for(let i=0;i<10;i++){
    const a=Math.random()*Math.PI*2,s=35+Math.random()*130;
    parts.spawn(x,y,Math.cos(a)*s,Math.sin(a)*s,'#ffd600',0.25+Math.random()*0.2,1.5+Math.random()*1.5);
  }
  // Scintilla bianca centrale
  parts.spawn(x,y,0,-80,'#fff',0.18,2.5);
}
// Particelle di scarico motore — chiamate nel loop principale
function spawnEngineExhaust(){
  const ch=CHARACTERS[G.activeChar];
  const col=TRAIL_SKINS[Save.trailSkin]||ch.trailColor;
  for(let i=0;i<CFG.playerEngineParticles;i++){
    const spread=(Math.random()-0.5)*55;
    const spd=40+Math.random()*80;
    parts.spawn(P.x+P.w*0.08,P.y+P.h*0.5+spread,-spd,spread*0.3,col,0.1+Math.random()*0.18,1.5+Math.random());
  }
  if(P.thrusting){
    // Fiamma di spinta extra
    parts.spawn(P.x+P.w*0.04,P.y+P.h*0.78,-(80+Math.random()*60),(Math.random()-0.5)*40,'#ffeb3b',0.12,2.2);
  }
}
...
function drawObstacle(o){
  ...
  if(has){
    if(isTop){
      ctx.save();
      ctx.translate(x+o.w/2,o.h/2);ctx.scale(1,-1);
      ctx.drawImage(img,-o.w/2,-o.h/2,o.w,o.h+20);
      ctx.restore();
    }else{
      ctx.drawImage(img,x,Math.floor(o.y),o.w,canvas.height-o.y+20);
    }
    // Glow sul bordo di contatto — sempre, non solo moving
    const edgeY=isTop?o.h:Math.floor(o.y);
    const pulse=o.variant==='moving'?0.5+0.5*Math.sin((o.movePhase||0)*2):0.7;
    ctx.save();
    ctx.shadowColor=pal.edge;ctx.shadowBlur=18*pulse;
    ctx.strokeStyle=pal.edge;ctx.lineWidth=2.5;ctx.globalAlpha=0.55*pulse;
    ctx.beginPath();ctx.moveTo(x,edgeY);ctx.lineTo(x+o.w,edgeY);ctx.stroke();
    // Secondo strato glow più morbido
    ctx.shadowBlur=32;ctx.lineWidth=6;ctx.globalAlpha=0.18*pulse;
    ctx.beginPath();ctx.moveTo(x,edgeY);ctx.lineTo(x+o.w,edgeY);ctx.stroke();
    ctx.restore();
    return;
  }
  ...
  const pulse=o.variant==='moving'?0.5+0.5*Math.sin((o.movePhase||0)*2):1;
  ctx.save();
  if(isTop){
    const h=o.h;
...
function drawCoin(c){
  ctx.save();
  if(c.type==='coin'){
    const bounce=Math.sin(c.anim*5.5)*0.1+1,r=c.r*bounce;
    const hasMagnet=G.powerup?.id==='magnet';
    // Ring magnetico animato
    if(hasMagnet){
      const mr=getMagnetRange();
      const dx=P.hbX-c.x,dy=P.hbY-c.y,dist=Math.sqrt(dx*dx+dy*dy);
      if(dist<mr){
        const proximity=1-(dist/mr);
        ctx.strokeStyle='#ffeb3b';ctx.lineWidth=1;ctx.globalAlpha=proximity*0.3*Math.abs(Math.sin(c.anim*8));
        ctx.beginPath();ctx.arc(c.x,c.y,r+4+proximity*8,0,Math.PI*2);ctx.stroke();
      }
    }
    // Outer glow ring
    ctx.shadowColor='#ffeb3b';ctx.shadowBlur=20;
    ctx.strokeStyle='rgba(255,235,59,0.4)';ctx.lineWidth=2;
    ctx.beginPath();ctx.arc(c.x,c.y,r+4,0,Math.PI*2);ctx.stroke();
    // Corpo moneta gradiente
    const g=ctx.createRadialGradient(c.x-r*0.3,c.y-r*0.35,0,c.x,c.y,r);
    g.addColorStop(0,'#fffde7');g.addColorStop(0.3,'#ffee58');g.addColorStop(0.75,'#ffd600');g.addColorStop(1,'#e65100');
    ctx.fillStyle=g;ctx.globalAlpha=1;
    ctx.beginPath();ctx.arc(c.x,c.y,r,0,Math.PI*2);ctx.fill();
    // Highlight
    ctx.fillStyle='rgba(255,255,255,0.65)';ctx.shadowBlur=0;
    ctx.beginPath();ctx.arc(c.x-r*0.28,c.y-r*0.28,r*0.28,0,Math.PI*2);ctx.fill();
    // Simbolo
    ctx.fillStyle='rgba(230,150,0,0.7)';ctx.font=`bold ${Math.round(r*1.1)}px serif`;
    ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('✦',c.x,c.y+0.5);
  }else{
    const pu=POWERUPS.find(p=>p.id===c.type)||POWERUPS[0];
    const bounce=Math.sin(c.anim*4)*0.15+1;
    // Aura esterna pulsante
    ctx.shadowColor=pu.color;ctx.shadowBlur=22;
    ctx.strokeStyle=pu.color;ctx.lineWidth=2;ctx.globalAlpha=0.35+0.25*Math.sin(c.anim*6);
    ctx.beginPath();ctx.arc(c.x,c.y,c.r*2.4*bounce,0,Math.PI*2);ctx.stroke();
    // Corpo
    ctx.fillStyle=pu.color+'44';ctx.globalAlpha=1;
    ctx.beginPath();ctx.arc(c.x,c.y,c.r*1.6*bounce,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle=pu.color;ctx.lineWidth=2.5;
    ctx.beginPath();ctx.arc(c.x,c.y,c.r*1.6*bounce,0,Math.PI*2);ctx.stroke();
    ctx.font=`${c.r*2}px serif`;ctx.textAlign='center';ctx.textBaseline='middle';
    ctx.shadowBlur=0;ctx.fillStyle=pu.color;
    ctx.fillText(pu.emoji,c.x,c.y+1);
  }
  ctx.restore();
}
...
//  SCUDO
// ═══════════════════════════════════════════════
let shieldAnim=0,shieldHitPulse=0;
function drawShield(){
  shieldAnim+=0.07;
  const r=Math.min(CFG.playerW,CFG.playerH)*0.72+Math.sin(shieldAnim)*3;
  ctx.save();
  // Onda di impatto
  if(shieldHitPulse>0){
    ctx.strokeStyle='#00e5ff';ctx.lineWidth=3;ctx.globalAlpha=shieldHitPulse*0.8;
    ctx.beginPath();ctx.arc(P.hbX,P.hbY,r+shieldHitPulse*22,0,Math.PI*2);ctx.stroke();
    shieldHitPulse=Math.max(0,shieldHitPulse-0.04);
  }
  // Outer glow
  ctx.shadowColor='#00e5ff';ctx.shadowBlur=28;
  ctx.strokeStyle='#00e5ff';ctx.lineWidth=2;ctx.globalAlpha=0.35+Math.sin(shieldAnim*1.8)*0.1;
  ctx.beginPath();ctx.arc(P.hbX,P.hbY,r+8,0,Math.PI*2);ctx.stroke();
  // Shell principale
  ctx.shadowBlur=18;ctx.lineWidth=3;ctx.globalAlpha=0.75+Math.sin(shieldAnim*2.2)*0.12;
  ctx.beginPath();ctx.arc(P.hbX,P.hbY,r,0,Math.PI*2);ctx.stroke();
  // Archi interni rotanti
  ctx.lineWidth=1.5;ctx.globalAlpha=0.25;
  for(let i=0;i<3;i++){
    const startA=shieldAnim*0.8+i*(Math.PI*2/3);
    ctx.beginPath();ctx.arc(P.hbX,P.hbY,r*0.88,startA,startA+Math.PI*0.45);ctx.stroke();
  }
  // Fill semitrasparente
  ctx.fillStyle='rgba(0,229,255,0.04)';ctx.globalAlpha=1;
  ctx.beginPath();ctx.arc(P.hbX,P.hbY,r,0,Math.PI*2);ctx.fill();
  ctx.restore();
}
...
function buildShop() {
  ...
  SHOP_ITEMS.forEach(item => {
    ...
    // 1. Costruiamo l'HTML di base (Icona + Nome + Descrizione)
    // Se l'oggetto è posseduto, mostriamo "✓ OK", altrimenti lasciamo vuoto
    row.innerHTML = `
      <div class="ret-item-icon">${item.icon}</div>
      <div class="ret-item-body">
        <div class="ret-item-name">${item.name}</div>
        <div class="ret-item-desc">${item.desc}</div>
      </div>
      ${owned ? '<div class="ret-item-price">✓ OK</div>' : ''} 
    `;
    
    // 2. Se non è posseduto, creiamo il bottone "BUY" con dentro il prezzo
    if (!owned) {
      const btn = document.createElement('button');
      btn.className = 'btn sm';
      
      // LA MAGIA È QUI: Inseriamo costo e moneta direttamente nel testo del bottone
...
function showMainMenu(){
  ...
  refreshMenuCoins();
  titleText.textContent='NEON DASH';
...
function update(dt){
  ...
  for(let o of obs.pool){
    ...
    if(circleAABB(hbX,hbY,hbR,o.x,o.y,o.w,o.h)){
      if(hasShield){G.powerup=null;G.shake=CFG.shakeIntensity*0.5;G.flashTimer=0.3;G.flashColor='#00e5ff';shieldHitPulse=1;spawnFire(P.hbX,P.hbY,12,['#00e5ff','#00bcd4','#fff']);Audio.shield();if(navigator.vibrate)navigator.vibrate([30,20,30]);}
      else{
  ...
  // Scarico motore continuo (più particelle quando spinge)
  if(G.state==='PLAYING'&&!Entry.active){
    spawnEngineExhaust();
  }
}
...
function drawDangerArrows(){
  for(let o of obs.pool){
    ...
    if(dist>0&&dist<180){
      ...
      const px=P.x+P.w+8;
      const ay=o.isScore?o.y+10:o.y+o.h-10;
      ctx.save();
      ctx.shadowColor=Zones.palette.edge;ctx.shadowBlur=12;
      ctx.globalAlpha=alpha*0.9*pulse;
      ctx.fillStyle=Zones.palette.edge;
      const sz=10+alpha*8;
      // Doppia freccia per maggiore visibilità
      for(let off of [-12,0]){
        const px2=px+off;
        ctx.beginPath();
        if(o.isScore){ctx.moveTo(px2,ay-sz);ctx.lineTo(px2+sz*1.3,ay);ctx.lineTo(px2,ay+sz);}
        else         {ctx.moveTo(px2,ay+sz);ctx.lineTo(px2+sz*1.3,ay);ctx.lineTo(px2,ay-sz);}
